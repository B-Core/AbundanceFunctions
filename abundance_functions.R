...
plot_raw_v_norm = function(linear_scale_raw_mat_with_rownames, str_of_raw_name=NULL, log2_scale_norm_mat_with_rownames, str_of_norm_name=NULL, xrng_vec=range(range(linear_scale_raw_mat_with_rownames, na.rm=T), range(2^log2_scale_norm_mat_with_rownames, na.rm=T)), yrng_vec=log2(xrng_vec), save_as_png=F, string_to_lead_file_name_with=NULL, color_by_vec_of_str=F, color_vec_of_strings=NULL){
  if(save_as_png==TRUE){
...
gene_pcaplot <- function(exprdat,sampleid,groupdat=NULL,colorfactor=NULL,shapefactor=NULL, plot_sampleids=TRUE, pcnum=1:2, plottitle = "PCA Plot") {
...
MA_plot_two_samples_same_treatment = function(Abundance_mat, vec_of_treatment_categories, yrng=NULL , xrngc=range(Abundance_mat, na.rm=T), gsub_str='A_11H1.*', main=NULL) {
  ...
  for(uniq_cntr in 1:length(uniq_tmnt)){
    ...
    if(length(cols_of_interest)>1){
      ...
      plot(x=xe, y=ye, xlab=paste0('Mn. lg2 ', gsub(gsub_str,'',colnames(Abundance_mat)[rand_two_to_get[1]]), " and ", gsub(gsub_str,'',colnames(Abundance_mat)[rand_two_to_get[2]])), ylab=paste0('Lg ratio ', gsub(gsub_str,'',colnames(Abundance_mat)[rand_two_to_get[1]]), "/", gsub(gsub_str,'',colnames(Abundance_mat)[rand_two_to_get[2]])), cex.lab=1.5, pch='.', col='blue', ylim=yrng, main=paste(main, uniq_tmnt[uniq_cntr], sep=': '))
...
complex_process_probes = function(probe_mat,probe_ID_vec, custom_ID_vec, quantile_to_remove=0.99999){
...
Noramlization_wrap = function(wd_path_str=params$proj_path, array_type_str=params$array_type, custom_probeset_mapping_needed = F, probe_ID_vec=NULL, custom_ID_vec=NULL){
  setwd(wd_path_str)
